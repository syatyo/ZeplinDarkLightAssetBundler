...
final class XCAssetsTests: XCTestCase {
    ...
    func testAssets() {
        ...
        XCTAssertEqual(xcassets.items.count, 5)
        XCTAssertEqual(xcassets.items.filter { $0 == .colorset(.empty()) }.count, 2)
        XCTAssertEqual(xcassets.items.filter { $0 == .imageset(.empty()) }.count, 2)
    }
